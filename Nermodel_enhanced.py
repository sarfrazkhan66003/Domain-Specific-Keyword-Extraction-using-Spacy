...
f = open("data_enhanced.txt", "r", encoding='cp1252')
paragraph = f.read()

#reading label data
labels = pd.read_csv(r'labels_enhanced.csv')
...
print(f"Total training examples: {len(outer_list)}")
...
if len(doc.ents) > 0:
    ...
    print("\nTip: Try text containing food, cloth, or technology terms like:")
    print("  - 'I bought tomatoes and spinach'")
    print("  - 'She wore a blue shirt and jeans'")
    print("  - 'Python is used for Machine Learning and Data Science'")

# Optional: Visualize entities
try:
    spacy.displacy.render(doc, style="ent", page="true")
except:
    print("\nVisualization skipped (no entities found)")
